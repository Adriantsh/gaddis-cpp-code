// This progam demonstrates the binarySearch function, which
...
int main()
{
    ...
    int results;    // To holde the search results
    int empID;      // To hold an employee ID

    // Get an employee ID to search for.
    cout << "Enter the employee IS you wish to search for: ";
    cin >> empID;
...
// found, its array subscript if returned. Otherwise, -1 is         *
...
int binarySearch(const int array[], int size, int value)
{
    ...
    while (!found && first <= last)
    {
        ...
        else if (array[middle] > value)     // If value is in lower hald
            last = middle - 1;
        else
